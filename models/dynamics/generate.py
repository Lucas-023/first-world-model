...
def load_models(vqvae_path, dynamics_path):
    ...
    vqvae.load_state_dict(torch.load(vqvae_path, map_location=DEVICE, weights_only=True)["model_state_dict"])
    vqvae.eval()
    ...
    dynamics.load_state_dict(torch.load(dynamics_path, map_location=DEVICE, weights_only=True)["model_state_dict"])
    dynamics.eval()
...
def generate_dream(vqvae, dynamics, priming_images, frames_to_generate=30):
    # Passa as imagens iniciais pelo Encoder e Quantizador
    z = vqvae.encoder(priming_images)
    ...
    # 🎮 PLANO DE JOYSTICK
    joystick_plan = [0] * 5 # Contexto inicial (NOOP)
    joystick_plan += [2] * (frames_to_generate // 2) # Metade do tempo Cima
    joystick_plan += [3] * (frames_to_generate - frames_to_generate // 2) # Metade do tempo Baixo
    
    plan_tensor = torch.tensor(joystick_plan, dtype=torch.long).to(DEVICE)
    
    print(f"🔮 Imaginando {frames_to_generate} frames de futuro...\n")
    pbar = tqdm(range(tokens_to_generate), desc="Gerando tokens")
    ...
    for i in pbar:
        ...
        # O modelo agora retorna Pixels, Rewards e Dones
        logits_pixels, logits_rewards, logits_dones = dynamics(context, actions=current_actions)
        
        # ------------------------------------------------------------
        # 🔍 AVALIAÇÃO DE RECOMPENSA E FIM DE JOGO
        # Verifica as previsões a cada final de frame (a cada 64 tokens)
        # ------------------------------------------------------------
        if i % 64 == 63:
            pred_done = torch.argmax(logits_dones[0, -1, :]).item()
            pred_reward = torch.argmax(logits_rewards[0, -1, :]).item()
            
            # Traduz a recompensa
            if pred_reward == 0:
                recompensa_str = "Ponto Inimigo! 🔴 (-1)"
            elif pred_reward == 2:
                recompensa_str = "Meu Ponto! 🟢 (+1)"
            else:
                recompensa_str = "Neutro ⚪ (0)"
            
            # Traduz o Done
            done_str = "Fim de Jogo! 🛑" if pred_done == 1 else "Rodando... ⏳"
            
            frame_atual = (i // 64) + 1
            # tqdm.write garante que o print não quebre a barra de progresso visualmente
            tqdm.write(f"▶️ Frame Imaginado {frame_atual:02d}/{frames_to_generate} | Status: {done_str} | Recompensa: {recompensa_str}")
        
        # ------------------------------------------------------------
        # GERAÇÃO DO PRÓXIMO PIXEL
        # ------------------------------------------------------------
        next_token_logits = logits_pixels[0, -1, :]
...
if __name__ == "__main__":
    ...
    DYNAMICS_CKPT = "models/DYNAMICS_PONG/dynamics_ckpt.pt"
